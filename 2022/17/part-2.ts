...
for (let i = 0; i < ITERATIONS; i++) {
  ...
  const initHeight = getMaxY(chamber);

  const startX = 2;
  const startY = initHeight + 4;
  ...
  height = getMaxY(chamber);
  const diff = height - initHeight;
  increases.push(diff);
}
...
// look at this output to determine where we are in the cycle
fs.writeFileSync('./levels.json', JSON.stringify(increases));
